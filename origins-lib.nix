pkgs: lib:
pkgs.lib // rec {
  andActions = actions: assert builtins.isList actions; {
    type = origins:and;
    inherit actions;
  };
  andConds = conditions: assert builtins.isList conditions; {
    type = origins:and;
    inherit conditions;
  };
  orConds = conditions: assert builtins.isList conditions; {
    type = origins:or;
    inherit conditions;
  };
  command = command: assert builtins.isString command; {
  ...
  mkPower =
    ...
    data:
      data // { inherit name type description; };
  mkHiddenPower =
    type:
      assert builtins.isString type;
    data:
      data // { inherit type; hidden = true; };
  mkMulti =
    name:
